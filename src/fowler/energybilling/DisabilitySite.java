...
public class DisabilitySite extends Site {
	...
	/*
	 * public Dollars charge() { int i; for (i = 0; _readings[i] != null; i++) ;
	 * // JK i - 1 is the last reading, i-2 the one before int usage =
	 * _readings[i - 1].amount() - _readings[i - 2].amount(); Calendar end =
	 * _readings[i - 1].date(); Calendar start = _readings[i - 2].date();
	 * start.set(Calendar.DAY_OF_MONTH, (start.get(Calendar.DAY_OF_MONTH) + 1));
	 * // set // to // beginning // of // period return charge(usage, start,
	 * end); }
	 */
	 ...
	 * 0) || ((arg.get(Calendar.YEAR) + 1900) % 400 == 0))) { result++; } return
	 * result; }
	 */
